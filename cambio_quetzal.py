...
from funcionesjo import mes_by_ordinal, hoy, year_ago, ultimo_dia_del_mes
...
def cambio_quetzal(fecha_hasta="", fecha_desde="") -> list[tuple]:
    ...
    if len(fecha_hasta) == 0:
        ...
        FECHA_REPORTE = ultimo_dia_del_mes(fecha=fecha_hasta, formato=FORMATO)
    if len(fecha_desde) == 0:
        FECHA_ANTERIOR = year_ago(fecha=FECHA_REPORTE, formato=FORMATO, inicio_de_mes=True)
    else:
